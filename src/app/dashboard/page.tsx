...
import type { OtisakExamWithSubject, OtisakAttemptWithExam } from '@/lib/db/otisak-types';
...
export default function DashboardPage() {
  ...
  const handleJoinExam = (examId: string) => router.push(`/exam/${examId}`);
  ...
  const handleStartPractice = async (examId: string) => {
    ...
    try {
      ...
      const data = await res.json();
      router.push(data.redirect_url || `/exam/${data.child_exam.id}`);
    } catch {
                          ...
                          {filteredHistory.map((attempt) => {
                            ...
                            return (
                              <div key={attempt.id}
                                ...
                                onClick={() => router.push(`/exam/${attempt.exam_id}/results`)}
                              >
