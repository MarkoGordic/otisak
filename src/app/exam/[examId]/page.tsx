...
export default function ExamPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!examRes.ok) { if (mounted) router.push('/dashboard'); return; }
  ...
  const handleFinish = useCallback(async (method: 'manual' | 'timeout' = 'manual') => {
    ...
    try {
      ...
      if (res.ok) router.push(`/exam/${examId}/results`);
    } catch (e) {
      console.error('Failed to submit:', e);
      setPhase('exam');
    }
  }, [phase, questions, answers, textAnswers, examId, router]);
